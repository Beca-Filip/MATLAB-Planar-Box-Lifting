...
% Define which trial we should take (THIS SHOULD BE LOOKED UP IN THE DOC
% SCRIPT)
simParam.TrialNumber = 1;

%% Create flags and parameters for the running of this script

% Create a flag for re-generating the cost functions and cosntraint
% functions
scriptParam.RegenerateCost = false;
scriptParam.RegenerateConstraints = false;

% Create a flag for saving graphs
scriptParam.SaveGraphs = true;

% Create a prefix for saving graphs
scriptParam.SavePrefix = [strrep(load_filename, '_50_ConstraintPoints', ''), '_allSamples_'];

% Create a saving path for graphs
scriptParam.SavePath = '../../data/Produced-Graphs/allSamples-Inverse-Optimization/';
scriptParam.SavePathFigs = '../../data/Produced-Graphs/allSamples-Inverse-Optimization/matFigs/';

% Create a file format suffix
scriptParam.SaveFormat = '.png';

%% Extract optimal solution from Spline Model DOC and prepare it for the IOC with all samples

% Load parameters to be defined
...
x_knots_star = Storage.Results.x_star;

% Get the trajectory knots from DOC interpolaton
q1_knot_star = x_knots_star(1 : itpParam.NumControlPoints);
q2_knot_star = x_knots_star(itpParam.NumControlPoints + 1 : 2*itpParam.NumControlPoints);
q3_knot_star = x_knots_star(2*itpParam.NumControlPoints + 1 : 3*itpParam.NumControlPoints);
...
qdqddq1_star = splineCoefToTrajectory(itpParam.KnotValues, c1_star, timeParam.TimeVectors, 2);
qdqddq2_star = splineCoefToTrajectory(itpParam.KnotValues, c2_star, timeParam.TimeVectors, 2);
qdqddq3_star = splineCoefToTrajectory(itpParam.KnotValues, c3_star, timeParam.TimeVectors, 2);

q_star = [qdqddq1_star(1, :); qdqddq2_star(1, :); qdqddq3_star(1, :)];
dq_star = [qdqddq1_star(2, :); qdqddq2_star(2, :); qdqddq3_star(2, :)];
ddq_star = [qdqddq1_star(3, :); qdqddq2_star(3, :); qdqddq3_star(3, :)];

% Get the equivalent optimal solution vector the approach with all samples
x_star = reshape(q_star.', 1, []);

%% Regenerate the cost and constraint functions with loaded normalisation


% If we want to regenerate cost
if scriptParam.RegenerateCost
    % Load cost function normalisation
    load('../../data/3DOF/allSamples/CostFunctionNormalisation.mat');
    % Store it in optParam structure
    optParam.CostFunctionNormalisation = CostFunctionNormalisation;
    % Clear unnecessary variable
    clear CostFunctionNormalisation
    % Suppress warnings about compiler version not being supported
    warning('off','all');
...
% If we want to regenerate constraints
if scriptParam.RegenerateConstraints
    % Suppress warnings about compiler version not being supported
    warning('off','all');
    % Generate nonlinear constraint computables
...
[A_star, b_star, Aeq_star, beq_star] = allSamplesGenerateLinearConstraintMatrices(timeParam, optParam, modelParam);

% Generate upper and lower bounds for our direct optimization variables
